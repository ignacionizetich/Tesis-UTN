...
public class ApiAdminController {
  ...
  @Operation(
    ...
    description = "Deshabilita un usuario común por su ID. No permite deshabilitar cuentas ADMIN o ROOT " +
      "(para eso existe /users/{id}/disable-admin, exclusivo de ROOT)."
  )
  @ApiResponses({
    @ApiResponse(
      responseCode = "200",
  ...
  @Operation(
    ...
    description = "Habilita un usuario común por su ID."
  )
  @ApiResponses({
    @ApiResponse(
      responseCode = "200",
  ...
  // =========================================================================
  // Endpoints exclusivos de ROOT: gestión de cuentas ADMIN.
  // El class-level @PreAuthorize("hasRole('ADMIN')") queda sobreescrito acá
  // a nivel método con una authority más estricta.
  // =========================================================================

  @Operation(
    summary = "Crear usuario administrador",
    description = "Crea un nuevo usuario con permisos de administrador. Exclusivo de ROOT."
  )
  @ApiResponses({
    @ApiResponse(
      responseCode = "200",
  ...
  @Operation(
    ...
    description = "Deshabilita una cuenta ADMIN por su ID. Exclusivo de ROOT. Las cuentas ROOT no se " +
      "pueden deshabilitar desde la aplicación."
  )
  @PreAuthorize("hasAuthority('ROLE_ROOT')")
  @PutMapping("/users/{id}/disable-admin")
  public ResponseEntity<?> disableAdmin(@PathVariable Long id) {
    adminService.disableAdmin(id);
    return ResponseEntity.ok("administrador deshabilitado correctamente");
  }

  @Operation(
    summary = "Habilitar administrador",
    description = "Habilita una cuenta ADMIN por su ID. Exclusivo de ROOT."
  )
  @PreAuthorize("hasAuthority('ROLE_ROOT')")
  @PutMapping("/users/{id}/enable-admin")
  public ResponseEntity<?> enableAdmin(@PathVariable Long id) {
    adminService.enableAdmin(id);
    return ResponseEntity.ok("administrador habilitado correctamente");
  }
