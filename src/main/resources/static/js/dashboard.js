document.addEventListener("DOMContentLoaded", () => {
    ...
    if (role === "ADMIN" && adminPanelLink) {
        ...
        adminPanelLink.addEventListener('click', async (e) => {
            e.preventDefault();
            try {
                const response = await fetchWithAuth('/api/admin/check-access', {});
                if (response.ok) {
    ...
    // --- Cargar datos de usuario y guardarlos en localStorage ---
    let userData = JSON.parse(localStorage.getItem("userData"));
    async function cargarUserData() {
        const response = await fetchWithAuth("/api/user/data", { method: "GET" });
        if (response.ok) {
    ...
    async function mostrarDatosUsuario() {
        ...
        if (userAliasEl) userAliasEl.textContent = `Alias: ${userData.alias}`;
    ...
    function actualizarSaldo() {
        ...
        fetchWithAuth(`/api/accounts/${accountId}/showBalance`, { method: "GET" })
            .then(res => {
    ...
    if (aliasButton) {
        aliasButton.addEventListener("click", () => {
            let userData = JSON.parse(localStorage.getItem("userData"));
            const alias = userData.alias || "No disponible";
            const cvu = userData.cvu || "No disponible";
    ...
    let cuentaOrigenId = localStorage.getItem('accountId');
    ...
    confirmarTransferenciaBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetchWithAuth(`/api/transactions/${cuentaOrigenId}/transfer/${cuentaDestinoId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    openProfileBtn.addEventListener('click', async (e) => {
        ...
        lastNameDisplay.textContent = userData.lastName || "";
        dniDisplay.textContent = userData.dni || "";
        emailDisplay.textContent = userData.email || "";
        usernameDisplay.textContent = userData.username || "";
        aliasDisplay.textContent = userData.alias || "";
        cvuDisplay.textContent = userData.cvu || "";
...
// --- Editar username con ícono de lápiz ---
    ...
    if (toggleBtn && balanceEl && eyeIcon) {
        toggleBtn.addEventListener("click", () => {
            balanceVisible = !balanceVisible;
            if (balanceVisible) {
                balanceEl.textContent = localStorage.getItem("lastBalance") || "0.00";
                eyeIcon.classList.remove("fa-eye-slash");
                eyeIcon.classList.add("fa-eye");
            } else {
                localStorage.setItem("lastBalance", balanceEl.textContent);
